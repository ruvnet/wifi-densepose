...
### Fixed
- **OpenISAC/RF-direct now fails closed at the trust boundary.** The bridge pairs raw range-Doppler data with same-frame metadata before forwarding a bounded, versioned `ruview.rf_observation` payload; it no longer converts signal peaks or SNR into motion, targets, occupancy, pose, or vital signs. The sensing server accepts only loopback RF input, validates schema/version/sequence/configuration/freshness, rejects duplicate and out-of-order observations, marks the source offline after five seconds, and exposes RF data as diagnostics with an explicit unverified capability manifest. Unknown `--source` values now fail instead of falling back to simulation, Docker no longer publishes unauthenticated RF UDP ports, and replay/DoS/staleness regression coverage was added.
