#!/usr/bin/env python3
"""UNVALIDATED HISTORICAL EXPERIMENT — not a RuView sensing producer.

This archived script emits a deprecated, unversioned feature shape containing
unvalidated human-sensing semantics. The current ``rf-direct`` boundary rejects
it. Keep it only for reproducing historical CW experiments; use
``scripts/openisac_to_ruview_bridge.py`` for supported observation transport.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="acknowledge that this archived experiment is not a supported RuView producer",
    )
...
def main() -> int:
    ...
    if not args.allow_unvalidated_output:
        raise SystemExit(
            "archived experiment disabled: pass --allow-unvalidated-output only for isolated, "
            "labelled research; use scripts/openisac_to_ruview_bridge.py for rf-direct"
        )
