...
- **multistatic** (ADR-029) — treat every authorized or passively observed WiFi link in a controlled environment as a candidate bistatic radar pair, then fuse. `v2/crates/wifi-densepose-signal/src/ruvsense/multistatic.rs` (attention-weighted fusion, geometric diversity), `phase_align.rs` (iterative LO phase-offset, circular mean), `multiband.rs`, `coherence.rs` / `coherence_gate.rs` (Z-score scoring; Accept / PredictOnly / Reject / Recalibrate).
...
- **tomography** — `ruvsense/tomography.rs` reconstructs a voxel occupancy grid via an ISTA L1 solver (sparse — most voxels empty); pair with cross-viewpoint geometry for experimental sparse volumetric occupancy reconstruction. RuVector solver crates back the 114→56 subcarrier sparse interpolation.
...
- **intention** — `ruvsense/intention.rs`; research hypothesis: pre-movement features may appear 200–500 ms ahead under controlled conditions.
- **adversarial** — `ruvsense/adversarial.rs`, rejects physically impossible signals + cross-checks multi-link consistency.
- **security** (ADR-032, multistatic mesh hardening) — using ambient / third-party-observed links and pooling links across a mesh expands the attack surface; restrict experiments to authorized environments. Mitigations: `adversarial.rs` + `coherence_gate.rs` quarantine (Reject / Recalibrate) + Ed25519 witness chain (ADR-028). Run a security review (`docs/security-audit-wasm-edge-vendor.md`); see `/ruview-verify`.
