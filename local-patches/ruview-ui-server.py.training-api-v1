...
def _start_training_request(kind: str, body: dict) -> tuple[int, dict]:
    ...
    if kind in {"supervised", "pretrain"} and not dataset_ids:
        return 400, {
            "status": "error",
            "error": "missing dataset",
            "message": "Select at least one CSI recording before starting training.",
        }
    ...
    with TRAINING_LOCK:
        TRAINING_STATE.update({
            ...
            "eta_secs": None,
            "phase": f"{kind}_queued",
            "message": "Desktop bridge validated the request. Run the full sensing-server trainer for RVF export.",
            "config": config,
