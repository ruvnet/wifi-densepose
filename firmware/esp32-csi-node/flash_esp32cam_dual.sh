...
idf.py -B "${BUILD_DIR}" \
  ...
  -D "SDKCONFIG_DEFAULTS=sdkconfig.defaults.esp32cam-dual" \
...
echo "ESP32-CAM RuView dual CSI + MJPEG firmware flashed on ${PORT}."
echo "Provision it next, for example:"
echo "  python provision.py --port ${PORT} --chip esp32 --node-id 3 --ssid <ssid> --password <password> --target-ip <deck-ip> --target-port 5005"
echo "After it joins WiFi, open:"
echo "  http://<esp32cam-ip>:8032/cam"
